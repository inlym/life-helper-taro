...
const LF = '\n'
const appKey = environments.aliyunAppKey
const appSecret = environments.aliyunAppSecret
...
export const requestInterceptor: (method: Method) => void | Promise<void> = (method) => {
  ...
  const pathAndParameters = paramsPairList.length === 0 ? method.url : method.url + '?' + paramsPairList.join('&')

  const stringToSign =
    httpMethpd + LF + ContentType.APPLICATION_JSON + LF + contentMd5 + LF + ContentType.APPLICATION_JSON + LF + LF + headersStr + LF + pathAndParameters
